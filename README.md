...
[![Build Status](https://travis-ci.com/KristofferC/Records.jl.svg?branch=master)](https://travis-ci.com/KristofferC/Records.jl) [![codecov](https://codecov.io/gh/KristofferC/Records.jl/branch/master/graph/badge.svg)](https://codecov.io/gh/KristofferC/Records.jl)

`Records` is a small Julia package that is used to generate formatted output from timings made in different sections of a program.
It's main functionality is the `@recordit` macro, similar to the `@time` macro in Base except one also assigns a label to the code section being timed.
...
# Create a Record, this is the main type that keeps track of everything.
const to = Record()

# Time a section code with the label "sleep" to the `Record` named "to"
@recordit to "sleep" sleep(0.02)

# Create a function to later time
rands() = rand(10^7)

# Time the function, @recordit returns the value being evaluated, just like Base @time
rand_vals = @recordit to "randoms" rands();

# Nested sections (sections with same name are not accumulated
# if they have different parents)
function time_test()
    @recordit to "nest 1" begin
        sleep(0.1)
        # 3 calls to the same label
        @recordit to "level 2.1" sleep(0.03)
        @recordit to "level 2.1" sleep(0.03)
        @recordit to "level 2.1" sleep(0.03)
        @recordit to "level 2.2" sleep(0.2)
    end
    @recordit to "nest 2" begin
        @recordit to "level 2.1" sleep(0.3)
        @recordit to "level 2.2" sleep(0.4)
    end
end
...
disable_timer!(to)
@recordit to "not recorded" sleep(0.1)
enable_timer!(to)
...
for i in 1:100
    @recordit to "sleep" sleep(0.01)
end

# Can also annotate function definitions
@recordit to funcdef(x) = x
...
@recordit to "nest 1" begin
    sleep(0.1)
    @recordit to "level 2.1" sleep(0.1)
    for i in 1:20; @recordit to "level 2.2" sleep(0.02); end
end
@recordit to "nest 2" begin
    for i in 1:30; @recordit to "level 2.1" sleep(0.01); end
    @recordit to "level 2.2" sleep(0.1)
end
```
...
It is possible to flatten this timer using the `Records.flatten` function that accumulates the data for all sections with identical labels:

```julia
julia> to_flatten = Records.flatten(to);
...
```julia
julia> to1 = Record(); to2 = Record();

julia> @recordit to1 "outer" begin
           @recordit to1 "inner" begin
                sleep(1)
           end
       end

julia> @recordit to2 "outer" begin
           sleep(1)
       end
...
For instance, use thread-local `Record` objects that are merged at custom merge points
...
Any `Record` can be indexed with the name of a section which returns a new `Record` with that section as the "root". For example:
...
@recordit to "nest 1" begin
    @recordit to "nest 2" begin
        @recordit to "nest 3.1" sleep(0.1)
        @recordit to "nest 3.2" sleep(0.1)
        @recordit to "nest 3.3" sleep(0.1)
    end
    sleep(0.3)
end
```
...
```julia
julia> Records.ncalls(to["nest 1"])
1

julia> Records.time(to["nest 1"]["nest 2"])
350441733

julia> Records.allocated(to["nest 1"]["nest 2"])
5280
```
...
```julia
julia> Records.tottime(to)
604937208

julia> Records.totallocated(to)
7632
```
...
all the functions and macros that do not take a `Record` instance and then use a global timer defined in the package.
...
The default timer object can be retrieved with `Records.get_defaulttimer()`.

## Measuring time consumed outside `@recordit` blocks
...
The `Records.complement!` function can be used to modify a timer and add
...
In order to complement the default timer simply call `Records.complement!()`.
...
Timers may be converted to a nested set of dictionaries with the (unexported) `Records.todict` function. This can be used to serialize a timer as JSON, for example.

```julia
julia> to = Record();

julia> @recordit to "nest 1" begin
           sleep(0.1)
           @recordit to "level 2.1" sleep(0.1)
           for i in 1:20; @recordit to "level 2.2" sleep(0.02); end
       end

julia> Records.todict(to)
...
julia> JSON3.write(Records.todict(to))
...
It is sometimes desireable to be able "turn on and off" the `@recordit` macro, for instance you may wish to instrument a package with `@recordit` macros, but then not deal with the overhead of the timings during normal package operation.
To enable this, we provide the `@recordit_debug` macro, which wraps the `@recordit` macro with a conditional, checking if debug timings have been enabled.
...
If a user calls `Records.enable_debug_record(<module>)`, the `<module>.recordit_debug_enabled()` method will be redefined, causing all dependent methods to be recompiled within that module.
This may take a while, and hence is intended only for debugging usage, however all calls to `@recordit_debug` (within that Module) will thereafter be enabled.
...
This package is inspired by the `Record` class in [deal.ii](https://dealii.org/).
